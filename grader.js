...
var cheerioHtmlUrl = function(htmlstr) {
...
var checkHtmlBuilder = function(loader) {
...
if(require.main == module) {
    program
        ...
        .parse(process.argv);
    if (program.file) {
        var checkHtml = checkHtmlBuilder(cheerioHtmlFile);
    ...
    if (program.url) {
        rest.get(program.url).on('complete', function(result) {
            ...
            var checkHtml = checkHtmlBuilder(cheerioHtmlUrl);
            var checkJson = checkHtml(result, program.checks);
            var outJson = JSON.stringify(checkJson, null, 4);
            console.log(outJson);
            fs.writeFileSync("part3.txt", outJson);
        });
    }
} else {
